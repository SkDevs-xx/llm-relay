...
| `RELAY_LOG` | `(未設定)` | `1` で中継1件ごとに入力（渡した内容）と出力（外部モデルのテキスト・tool 呼び出し引数）を router.log に1行記録。tool use の QA・自己申告に頼らない確認用。**会話内容を記録するので常用は避ける** |
...
**tool use の QA**: `RELAY_LOG=1` でルーターを起動すると、中継ごとに `router.log` に2行出ます:
