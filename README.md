...
| `RELAY_UPSTREAM_TIMEOUT_MS` | `90000`（90秒） | 上流が応答ヘッダを返さない／ストリームが無通信のままこの時間で中断。チャンク受信ごとにリセットするので健全な長ストリームは切らない |
| `RELAY_LOG` | `(未設定)` | `1` で中継1件ごとに入力（渡した内容）と出力（外部モデルのテキスト・tool 呼び出し引数）を **stderr** に出力。tool use の QA・自己申告に頼らない確認用。ファイルには書かない（`start-router` 経由では破棄）ので、見るには `RELAY_LOG=1 node src/router.mjs` のように直接起動して端末で確認する。**会話内容を出すので常用は避ける** |
